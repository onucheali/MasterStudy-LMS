...
class STM_LMS_Point_System {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_stm_lms_buy_bundle_for_points', array( $this, 'buy_bundle_for_points' ) );
		add_action(
			'admin_init',
			function () {
	...
	public static function get_label() {
		...
		return ( ! empty( $options['point_label'] ) ) ? $options['point_label'] : __( 'points', 'masterstudy-lms-learning-management-system-pro' );
	}
	...
	public static function bundle_price( $bundle_id ) {
		$points_price = get_post_meta( $bundle_id, 'stm_lms_bundle_points', true );

		if ( ! empty( $points_price ) || '0' === $points_price ) {
			return intval( $points_price );
		}
	...
	public function buy_bundle_for_points() {
		check_ajax_referer( 'stm_lms_buy_bundle_for_points', 'nonce' );

		$user      = STM_LMS_User::get_current_user();
		$user_id   = $user['id'];
		$bundle_id = intval( $_GET['bundle_id'] );

		$points_price = self::bundle_price( $bundle_id );
		$my_points    = intval( self::total_points( $user_id ) );
		$bundle_ids   = get_post_meta( $bundle_id, 'stm_lms_bundle_ids', true );

		if ( $points_price && $my_points >= $points_price && ! empty( $bundle_ids ) ) {
			foreach ( $bundle_ids as $course_id ) {
		...
		wp_send_json(
			array(
				'url' => esc_url( STM_LMS_User::user_page_url() . 'enrolled-courses/' ),
			)
		);
	}
}
