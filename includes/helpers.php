...
function masterstudy_lms_count_assignments_by_status( $user_assignments, $status ): int {
	return count(
		array_filter(
			$user_assignments,
			function( $assignment ) use ( $status ) {
...
function masterstudy_lms_timecode_to_string( $timecode ) {
	$hours   = floor( $timecode / 3600 );
	$minutes = floor( ( $timecode % 3600 ) / 60 );
	$seconds = $timecode % 60;

	return sprintf( '%02d:%02d:%02d', $hours, $minutes, $seconds );
}

function masterstudy_lms_string_to_timecode( $time = '00:00:00' ) {
	if ( empty( $time ) ) {
		return - 1;
	}
...
function masterstudy_lms_taxes_display() {
	$class = '\STM_LMS_Helpers';

	if ( class_exists( $class ) && is_callable( array( $class, 'taxes_display' ) ) ) {
		return $class::taxes_display();
	}
...
function masterstudy_lms_display_price_with_taxes( $amount, $user_id = null, $only_value = false ) {
	$val   = is_numeric( $amount ) ? (float) $amount : 0.0;
	$class = '\STM_LMS_Helpers';

	if ( class_exists( $class ) && is_callable( array( $class, 'display_price_with_taxes' ) ) ) {
		return $class::display_price_with_taxes( $val, $user_id, $only_value );
	}

	return $class::display_price( $val );
}

function masterstudy_lms_display_taxes_amount( $amount, $user_id = null, $only_value = false ) {
	$val   = is_numeric( $amount ) ? (float) $amount : 0.0;
	$class = '\STM_LMS_Helpers';

	if ( class_exists( $class ) && is_callable( array( $class, 'display_taxes_amount' ) ) ) {
		return $class::display_taxes_amount( $val, $user_id, $only_value );
	}

	return 0.0;
}

function masterstudy_lms_get_subscription_status_labels() {
	return array(
		'trialing'         => esc_html__( 'Trial', 'masterstudy-lms-learning-management-system-pro' ),
		'trial'            => esc_html__( 'Trial', 'masterstudy-lms-learning-management-system-pro' ),
		...
		'approval_pending' => esc_html__( 'Pending', 'masterstudy-lms-learning-management-system-pro' ),
		'on_hold'          => esc_html__( 'On Hold', 'masterstudy-lms-learning-management-system-pro' ),
		'completed'        => esc_html__( 'Completed', 'masterstudy-lms-learning-management-system-pro' ),
		'refunded'         => esc_html__( 'Refunded', 'masterstudy-lms-learning-management-system-pro' ),
		'paid'             => esc_html__( 'Paid', 'masterstudy-lms-learning-management-system-pro' ),
	);
}

function is_ms_lms_coupons_enabled(): bool {
	return method_exists( 'STM_LMS_Helpers', 'is_coupons_enabled' ) && STM_LMS_Helpers::is_coupons_enabled();
}

function masterstudy_get_current_account_slug(): string {
	if ( class_exists( 'STM_LMS_User_Menu' ) && method_exists( 'STM_LMS_User_Menu', 'get_current_account_slug' ) ) {
		return STM_LMS_User_Menu::get_current_account_slug();
	}
