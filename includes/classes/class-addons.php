...
class STM_LMS_Pro_Addons {
	...
	public static function update_addons_option( $addons ) {
		...
		if ( function_exists( 'stm_lms_user_assignments_table' ) && class_exists( '\MasterStudy\Lms\Pro\addons\assignments\Assignments' ) ) {
			...
			if ( function_exists( 'stm_lms_user_assignments_times_table' ) ) {
				stm_lms_user_assignments_times_table();
			}
		}

		if ( ! function_exists( 'stm_lms_subscription_tables' ) ) {
			$dir = STM_LMS_PRO_PATH . '/addons-plus/Subscriptions/Database';
