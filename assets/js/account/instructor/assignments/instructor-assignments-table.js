(function ($) {
    $(document).ready(function () {
        ...
        const tableLoaderTemplate = table.children('.masterstudy-loader').first();
        const noFoundContainer = $('.masterstudy-account-instructor-assignments-no-found__info')
        ...
        document.addEventListener('msfieldEvent', function(event) {
            ...
            switch ( event.detail.name ) {
                ...
                case 'status':
                    ...
                    break;
                case 'masterstudy-account-instructor-assignments__course-select-input':
                    course_id = fieldValue ?? ''
            }

            fetchData({per_page, search, status, page: 1, course_id}, true);
        });
        ...
        function notFound(maxPages) {
            if (!search && !status && !course_id && maxPages === 0) {
                table.toggle(false)
                noFoundContainer.toggleClass('masterstudy-account-utility_hidden', false)
                return
            } else {
                table.toggle(true)
                noFoundContainer.toggleClass('masterstudy-account-utility_hidden', true)
            }
        ...
        function addDataToTable(order, data, page, per_page) {
            ...
            tableItem.find('.masterstudy-tcell__data').each(function(i, cell) {
                ...
                switch(key) {
                    case 'title': {
                        $(cell).find('.masterstudy-tcell__title-value').text(value)
                        const courses = data['courses'] ?? []
                        const coursesHtml = relatedCourses(courses);
                        if (coursesHtml) {
                            $(cell).find('.masterstudy-table__list-no-course').addClass('hidden');
                            $(cell).find('.masterstudy-table__list').append(coursesHtml);
                        } else {
        ...
        function loader(isToHide = false) {
            if (isToHide) {
                ...
                if (prevLoader.length < 1) {
                    const loader = tableLoaderTemplate.length
                        ? tableLoaderTemplate.clone()
                        : $('.masterstudy-loader').first().clone();
