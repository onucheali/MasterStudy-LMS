...
function updateTable(reloadTable = false) {
    const gradesContainer = document.querySelector('.masterstudy-grades-instructor')
    gradesContainer.querySelector('.masterstudy-grades-instructor .masterstudy-grades-instructor-no-found__info')?.remove();
    gradesContainer.querySelector('.masterstudy-grades-instructor-table').style.display = 'block'
    const dataSrc = function (json) {
        if (!json.data || String(json.recordsTotal) === '0') {
            const template = document.getElementById('masterstudy-grades-instructor-no-records');
            if ( template ) {
                const clone = template.content.cloneNode(true);
                gradesContainer.querySelector('.masterstudy-grades-instructor-table').style.display = 'none'
